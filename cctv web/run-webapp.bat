...
REM Use port 50939 strictly as requested
set "PORT=50939"
echo %PORT%>PORT.txt
...
REM Stop any stale server already using this port or running this app, so old instances do not serve old pages.
powershell -NoProfile -Command "$p='%PORT%'; if ($p -match '^\d+$') { Get-NetTCPConnection -LocalPort $p -State Listen -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess -Unique | ForEach-Object { try { Stop-Process -Id $_ -Force -ErrorAction Stop; Write-Host ('Stopped process PID ' + $_ + ' on port ' + $p) } catch {} } }; Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -like '*uvicorn*' -and $_.CommandLine -like '*app.main:app*' } | ForEach-Object { try { Stop-Process -Id $_.ProcessId -Force -ErrorAction Stop; Write-Host ('Stopped stale uvicorn process PID ' + $_.ProcessId) } catch {} }"
